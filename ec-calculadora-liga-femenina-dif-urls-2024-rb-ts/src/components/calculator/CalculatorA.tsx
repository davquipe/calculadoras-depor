...
const CalculatorA = ({ allScores, defaultMatchDay }: CalculatorAProps) => {
  const middleIndex = Math.ceil(allScores.length - 1);
  ...
  const getMatchesFromSessionStorage = () => {
    ...
    let dataFromSessionStorage: string =
  ...
  const aperturaMatches =
    aperturaMatchesFromSessionStorage.length > 0
      ? restoreMatchesKeys(aperturaMatchesFromSessionStorage)
      : aperturaMatchesFromApi;

  const getMatchesByMatchDay = (matches: Match[]) => {
    const matchesByMatchDay = matches.reduce(
      (group: MatchByMatchDay, match: Match) => ({
        ...group,
  ...
  const getCurrentMatchDay = (matches: Match[]) =>
    typeof defaultMatchDay === "string"
      ? parseInt(defaultMatchDay)
      : getCurrentMatchDayFromApi(matches);

  const getCurrentMatchDayFromApi = (matches: Match[]) => {
    const matchWithCurrentDay = Object.values(matches).find(
      (match) => match.currentMatchDay === true
    );

    return typeof matchWithCurrentDay !== "undefined"
      ? matchWithCurrentDay.matchDay
      : minActiveMatchDay;
  };
  ...
  const breakTies = (arrayTeams: TeamData[], seasonId: number) => {
    arrayTeams.sort((a, b) => {
      if (a.seasons[seasonId].points === b.seasons[seasonId].points) {
        if (
          a.seasons[seasonId].goalsDifference <
          b.seasons[seasonId].goalsDifference
        ) {
          return 1;
        }
  ...
  const getStanding = (matches: Match[], teams: Team, seasonId: number) => {
    ...
    arrayTeams.sort((a, b) =>
      b.seasons[seasonId].points > a.seasons[seasonId].points ? 1 : -1
    );
  ...
  const activeMatchDayReducer = (
    state: ChangeMatchDayState,
    action: ChangeMatchDayAction
  ) => {
    switch (action.type) {
  ...
  const disableButton = (matchDay: number, value: number) =>
    matchDay === value ? true : false;

  const UsePersistForm = (matches: Match[]) =>
    sessionStorage.setItem(
      sessionStorageKey,
      JSON.stringify(matches.filter((match) => match !== null))
    );
  ...
  const resetMatches = () => {
    ...
    matchesNumbers.map((matchNumb) => {
      ...
      if (aperturaMatches[matchNum].fromApi === false) {
        ...
        aperturaMatches[matchNum].fromSessionStorage =
          defaultMatch.fromSessionStorage;
      }
    });
  ...
  const defaultActiveMatchDaysBlock =
    state.activeMatchDay <= halfMatchDay ? 0 : 1;
  const [activeMatchDaysBlock, setActiveMatchDaysBlock] = useState<number>(
    defaultActiveMatchDaysBlock
  );

  const handleActiveMatchDaysBlock = (selectedActiveMatchDaysBlock: number) =>
    setActiveMatchDaysBlock(selectedActiveMatchDaysBlock);
  return (
