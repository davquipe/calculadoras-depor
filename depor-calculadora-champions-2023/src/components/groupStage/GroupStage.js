...
//constants
import { epigraph, hiddenTitle, isPlayed, titleHome, urlFlags, sessionStorageKey } from '../../Constants';
...
const GroupStage = () => {
    ...
    const { scores, setKnockOutStage, setStage, knockOutStagePos } = useContext(GroupsContext);
    ...
    const getMatchesFromSessionStorage = () => {
        let matches = [];
        let dataFromSessionStorage = sessionStorage.getItem(sessionStorageKey) || ''
    ...
    const matches = (matchesFromSessionStorage.length > 0) ? restoreMatchesKeys(matchesFromSessionStorage) : matchesFromApi;
    ...
    const UserPersistForm = (matchesForPersist) => sessionStorage.setItem(sessionStorageKey, JSON.stringify(matches.filter(mch => mch !== null)))
    ...
    return (
        <section className="mt-0">
            <section className="text-center">
                <h1 className="text-center title-name">{hiddenTitle}</h1>
                    ...
                    <section className="col-12 d-flex justify-content-center">
                            <button 
                                ...
                                onClick={() => {
                                    ...
                                    handleSubmitD(onSubmitFormD({group: 'D'}))
                                }}
                            >
                                SIGUIENTE
                            </button>
